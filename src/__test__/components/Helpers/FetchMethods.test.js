...
const handlers = [
  rest.get('https://api.themoviedb.org/3/movie/577209', (req, res, ctx) => {
    return res(ctx.json({title: 'Thunder and Lightning', id: 12345}))
  }), 
  rest.get('https://api.themoviedb.org/3/discover/movie', (req, res, ctx) => {
    return res(ctx.json([{title: 'Thunder and Lightning', id: 123}, {title: 'Return of Thunder', id: 456}]));
  }),
  rest.get('https://api.themoviedb.org/3/genre/movie/list', (req, res, ctx) => {
    return res(ctx.json([{id: 28, name: 'Action'}, {id: 35, name: 'Comedy'}]));
  })
]
...
describe("fetchMovieDetials", () => {
  it('It fetches movie data', async () => {
    let movie = await fetchMovieDetails('onvehofdksd', 577209);
...
test('It fetches movies list', async () => {
  let list = await fetchMoviesList('onvehofdksd', '2009', '28', '1');
...
test('It should fetch genres list', async () => {
  let genres = await fetchGenreList('onvehofdksd');
